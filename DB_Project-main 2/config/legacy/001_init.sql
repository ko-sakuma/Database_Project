...
CREATE TABLE IF NOT EXISTS users (
	...
	user_password BINARY(60) NOT NULL, -- datatype for hashed password
...
CREATE TABLE IF NOT EXISTS user_address (
	...
	user_street VARCHAR(60) NOT NULL,
	user_country VARCHAR(60) NOT NULL,
	user_postcode VARCHAR(16) NOT NULL
);
...
CREATE TABLE IF NOT EXISTS product_condition (
	...
	product_condition_name VARCHAR(16) NOT NULL
);
...
ALTER TABLE users ADD CONSTRAINT fk_users_role FOREIGN KEY (user_role) REFERENCES user_role(user_role_id) ON DELETE CASCADE;

-- user_address table constraints:
ALTER TABLE user_address ADD CONSTRAINT fk_user_address_user_id FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE;
...
INSERT IGNORE INTO user_role VALUES (0, 'Buyer'), (1, 'Seller'), (2, 'Admin');

-- seed product_status table
INSERT IGNORE INTO product_status VALUES (0, 'Active'), (1, 'Ended'), (2, 'Sold');
-- retracted is maybe not even needed!!

-- seed product_category table (to be decided final category)
INSERT IGNORE INTO product_category VALUES (0, 'Red'), (1, 'White'), (2, 'Rose'), (3, 'Champagne');
